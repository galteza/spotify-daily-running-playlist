...
def dailyRunningPlaylist():
    ...
    for query in liked_songs_query:
        if query in saved_queries:
            song_tempo = saved_BPMs[saved_queries.index(query)]

            if 75 <= int(song_tempo) <= 85 or 150 <= int(song_tempo) <= 170:
                possible_running_songs_uri.append(liked_songs_uri[liked_songs_query.index(query)])

        elif not query in saved_queries:
            new_data = {'Query': [query]}
            pd.DataFrame(new_data).to_csv('BPMs.csv', mode='a', index=False, header=False)
            restart = True

            '''
            song_tempo = scrapeBPM(query, driver)
            # Updating the BPMs.csv file
            new_data = {'Query': [query],
                        'BPM': [song_tempo]}
            pd.DataFrame(new_data).to_csv('BPMs.csv', mode='a', index=False, header=False)
            '''
    ...
    if restart:
        return 'New Liked Songs detected, run again!'
